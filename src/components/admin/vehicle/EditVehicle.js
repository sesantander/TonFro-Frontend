...
class EditVehicle extends Component {
  constructor(props) {
    ...
    this.requiredText = ["title", "brand", "model", "vehicleNumber", "maxSeat"];
  ...
  getvehicleimei = (id) => {
    vehicle.getvehicleimei(id).then(res => {
      
      let data = res.data;
      if (data.status) {
        data = data.data; 
        let form = this.state.form;
        form['imei'] = (typeof data[0]!='undefined' && typeof data[0].imei!='undefined')?data[0].imei:'';
        this.setState({
          form: form
        });
  ...
  uplodaimage(formdata) {
    console.log('test sebastian',formdata)
    ...
    vehicle.update(formData).then(
      res => {
        ...
        this.setState({
         ...
         if(this.state.imei!=''){
            this.addimei({'imei':this.state.form.imei,'vehicleId':this.state.vehicleId});
          }
        let newform = new FormData();
        newform.append("vehicleId", this.state.vehicleId);
        newform.append("image",this.state.file)
       
        for (let v of newform) {
            ...
            <div className="box">
              ...
              <div className="box-body">
                ...
                <div className="row">
                  <div className="col-md-6">
                    <div className="form-group required">
                      <label className="control-label">IMEI</label>
